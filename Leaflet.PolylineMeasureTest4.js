...
/**
		...
		_mouseClick: function(e) {
            ...
            if (self._currentCircle) {
                ...
                self._currentCircle.on ('mousedown touchstart', self._dragCircle, self);
			}
        ...
        _dragCircle: function (e1) {
			...
			self._map.on ('mousemove touchmove', function (e2) {
                ...
                self._map.on ('mouseup touchend', function () { 
                    self._resetPathVariables();
                    self._map.off ('mousemove touchmove');
                    self._map.dragging.enable();
                    self._map.on ('mousemove', self._mouseMove, self);
                    self._map.off ('mouseup touchend');
                });
            });
            }
        }
